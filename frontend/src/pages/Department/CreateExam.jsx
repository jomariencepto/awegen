...
import '../Teacher/css/createexam.css';
...
function CreateExam() {
  ...
  const durationValue    = (watchedDuration !== '' && watchedDuration != null) ? watchedDuration : '';
  const passingValue     = (watchedPassing  !== '' && watchedPassing  != null) ? watchedPassing  : '';
  const scoreLimitValue  = targetScoreLimit === 0 ? '' : targetScoreLimit;
  const durationError    = durationValue !== '' && (Number(durationValue) < 1 || Number(durationValue) > 150);
  const passingError     = passingValue  !== '' && (Number(passingValue)  < 1 || Number(passingValue)  > 100);
  const scoreLimitError  = scoreLimitValue !== '' && (scoreLimitValue < 1 || scoreLimitValue > 150);

  const clampDuration = (v) => { const n = parseInt(v) || 0; if (n <= 0) return ''; return String(Math.min(n, 150)); };
  const clampPassing  = (v) => { const n = parseInt(v) || 0; if (n <= 0) return ''; return String(Math.min(n, 100)); };
  ...
  const onGenerateSubmit = async (data) => {
    ...
    if (parseInt(data.duration_minutes) > 150) { toast.error('Duration cannot exceed 150 minutes'); return; }
    if (parseInt(data.passing_score)    > 100) { toast.error('Passing score cannot exceed 100%');   return; }
  ...
  const onSaveExam = async () => {
    ...
    try {
      ...
      toast.success('Exam saved & auto-approved!');
  ...
  const saveButtonLabel = 'Save & Approve';
  ...
  // ── Render ──────────────────────────────────────────────────────────────────
  return (
    <div className="create-exam-container department-create-exam-page">
            ...
            <CardHeader>
              <div className="results-header">
                <div>
                  ...
                  <CardDescription>
                    Review your exam · it will be approved automatically on save
                  </CardDescription>
                </div>
                        ...
                        <div className="question-header">
                          ...
                          <div className="question-body">
                            <p className="question-text">
                              {(q.question_type === 'true_false' || q.question_type === 'identification')
