...
function SpecialExams() {
  ...
  const toggleSpecialStatus = async (examId, currentStatus) => {
    try {
      ...
      setExams(
        exams.map((exam) =>
          exam.exam_id === examId ? { ...exam, is_special: !currentStatus } : exam
        )
      );
    } catch (error) {
          ...
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-1">
                No special exams found
              </h3>
              <p className="text-gray-500">
                {searchTerm
                  ? 'Try adjusting your search'
                  : 'No exams have been marked as special yet'}
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <Card key={exam.exam_id}>
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  {exam.is_special && <Badge variant="default">Special</Badge>}
                </div>
